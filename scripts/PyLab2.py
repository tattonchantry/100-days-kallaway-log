...
def FirstDerApp(t, dx, an_sol):
    ...
    the center point:
        ...
        Higher Order Forward: f'(x_i) = (-3 * f'(x_i) + 4 * f(x_i+1) - f(x_i+2)) / 2 * dx
        Higher Order Backward: f'(x_i) = (3 * f'(x_i) - 4 * f(x_i-1) + f(x_i-2)) / 2 * dx
        Higher Order Centered: f'(x_i) = (f(x_i+1) - 2 * f(x_i) + f(x_i-1)) / 2 * dx
    Takes:
...
def FirstThreePrinter(dict_of_obs, dict_of_errors_and_dx):
    ...
    for cs in dict_of_obs:
        ...
        row = ['f\' fwrd | ', str(round(fst_f, 4)) + ' |', str(round(fst_f_err, 4)) + '|']
        print(row_format.format("", *row))
        row = ['f\' bwrd | ', str(round(fst_b, 4)) + ' |', str(round(fst_b_err, 4)) + '|']
        print(row_format.format("", *row))
        row = ['f\' cntr | ', str(round(fst_c, 4)) + ' |', str(round(fst_c_err, 4)) + '|']
        print(row_format.format("", *row))
        row = ['f\' hof | ', str(round(fst_hof, 4)) + ' |', str(round(fst_hof_err, 4)) + '|']
        print(row_format.format("", *row))
        row = ['f\' hob | ', str(round(fst_hob, 4)) + ' |', str(round(fst_hob_err, 4)) + '|']
        print(row_format.format("", *row))
        row = ['f\' hoc | ', str(round(fst_hoc, 4)) + ' |', str(round(fst_hoc_err, 4)) + '|']
        print(row_format.format("", *row))
...
def LastPrinter(dict_of_obs, dict_of_errors_and_dx):
    ...
    print(row_format.format("", *row))
    for cs in dict_of_obs:
        sec_deriv = SecondDerApp(dict_of_obs['Case 1'],
                                 dict_of_errors_and_dx[cs][0],
                                 dict_of_errors_and_dx[cs][1])
        row = ['∆x = ' + str(dict_of_errors_and_dx[cs][0]) + ' | ',
               str(round(sec_deriv, 4)) + ' |',
               str(round(sec_deriv - dict_of_errors_and_dx[cs][1], 4)) + '|']
        print(row_format.format("", *row))
